...
g21 <- ggplot(score_table_df, aes(x = Total)) +
  geom_histogram(breaks = seq(30, 200, by = 10), fill = "skyblue", color = "white") +
  scale_x_continuous(breaks = seq(30, 200, by = 10)) +
...
#頻度表
h <- hist(score_table_df$Total, breaks = seq(30, 200, by = 10), plot = FALSE)
...
#for (i in 1:nrow(score_table_df)) {
  ...
  g34 <- ggplot(score_table_df, aes(x = Total)) +
    geom_histogram(breaks = seq(30, 200, by = bin_width), fill = "skyblue", color = "white", closed = "right") +
    # ご提案の通り > と <= を用いたフィルタリング
    geom_histogram(data = score_table_df %>% filter(Total > bin_left & Total <= bin_right), 
                   breaks = seq(30, 200, by = bin_width), fill = "orange", color = "white", closed = "right") +
    scale_x_continuous(breaks = seq(30, 200, by = bin_width)) +
